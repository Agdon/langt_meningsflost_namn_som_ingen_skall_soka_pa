...
(define database-class;;The Database handles Items in the list items-list
  (class object%
    ...
    (define/public set-item-cost!
      (lambda (index inte)
        (send (cdr (assoc index items-list)) set-cost! inte)))
    ...
    (define/public get-item-cost
      (lambda (index)
        (send (cdr (assoc index items-list)) get-cost)))
    ...
    (define/public get-saldo-valv
      (lambda () saldo-valv))
    (define/public move-saldo-vaxel->saldo-valv;;Note positive if moving from stupan to valv
      (lambda (amount)
        (if (and (< amount saldo-vaxel) (> amount (- saldo-valv)))
            (begin
              (set! saldo-valv (+ saldo-valv amount))
              (set! saldo-vaxel (- saldo-vaxel amount)))
            (void))))
